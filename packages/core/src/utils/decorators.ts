...
import { appendCanvasElem } from './doms';
import { isFunction, throwError } from './base';
...
export function mixins(...list: unknown[]) {
  return function (target: ProxyConstructor): void {
...
export function addEnvContext(
  ...
  if (isWeb) {
    ...
    if (isFunction($el.getContext)) {
      ...
      // 需要创建canvas
      _canvas = appendCanvasElem($el, opts);
    }
  } else if (isWeapp) {
    // weapp
    // virtual canvas
